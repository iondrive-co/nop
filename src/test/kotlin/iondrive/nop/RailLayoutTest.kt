...
class RailLayoutTest {
    ...
    @Test
    fun `expanded opens a collapsed separator and leaves everything else alone`() {
        assertEquals(sep("Work"), sepC("Work", 3).expanded())
        assertEquals(3L, (sepC("Work", 3).expanded() as RailItem.Separator).id)
        assertEquals(proj("/p/a"), proj("/p/a").expanded())
    }
