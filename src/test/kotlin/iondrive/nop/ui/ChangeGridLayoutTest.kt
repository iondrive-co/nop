...
class ChangeGridLayoutTest {
    ...
    fun `single group fills the whole width in one row`() {
        val grid = ChangeGridMetrics.layout(1, 200.dp, wide, tall)
    ...
    fun `a few groups share one row, stretched to fill and never below natural`() {
        val grid = ChangeGridMetrics.layout(3, 200.dp, wide, tall)
        assertEquals(1, grid.rows)
    ...
    fun `columns wrap to a new row below before scrolling once they no longer fit across`() {
        ...
        val grid = ChangeGridMetrics.layout(6, 200.dp, wide, tall)
    ...
    fun `once vertical space is exhausted the grid scrolls horizontally`() {
        ...
        val grid = ChangeGridMetrics.layout(10, 200.dp, wide, 150.dp)
        assertEquals(1, grid.rows)
    ...
    fun `natural width is clamped up to the minimum`() {
        ...
        val grid = ChangeGridMetrics.layout(20, 50.dp, 300.dp, 120.dp)
        assertTrue(grid.scrollHorizontally)
        assertEquals(ChangeGridMetrics.MIN_COLUMN_WIDTH.value, grid.columnWidth.value, 0.01f)
    }

    @Test
    fun `natural width is clamped down to the maximum`() {
        val grid = ChangeGridMetrics.layout(10, 1000.dp, 500.dp, 120.dp)
        assertTrue(grid.scrollHorizontally)
        assertEquals(ChangeGridMetrics.MAX_COLUMN_WIDTH.value, grid.columnWidth.value, 0.01f)
    }

    @Test
    fun `no groups yields an empty grid`() {
        val grid = ChangeGridMetrics.layout(0, 200.dp, wide, tall)
    ...
    fun `degenerate zero-size constraints do not crash and stay sane`() {
        val grid = ChangeGridMetrics.layout(3, 200.dp, 0.dp, 0.dp)
        assertTrue(grid.columnsPerRow >= 1)
        assertTrue(grid.rows >= 1)
        assertTrue(grid.columnWidth.value >= ChangeGridMetrics.MIN_COLUMN_WIDTH.value)
    }
    ...
    fun `adding groups to a fixed area progresses stretch then wrap then scroll`() {
        ...
        for (n in 1..4) {
            val g = ChangeGridMetrics.layout(n, 200.dp, wide, tall)
        ...
        val wrapped = ChangeGridMetrics.layout(9, 200.dp, wide, tall)
        assertTrue(wrapped.rows in 2..3)
        assertFalse(wrapped.scrollHorizontally)
        // Beyond 4x3 capacity: scrolls horizontally.
        val overflow = ChangeGridMetrics.layout(13, 200.dp, wide, tall)
