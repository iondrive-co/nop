 ...
 *
 * nop never asks for a password and never handles a token. Both CLIs authenticate through a browser
 * OAuth flow they drive themselves; all nop does is start the right command with the account's home
 * in its environment, so whatever the flow writes lands in that account's directory and not in the
 * machine owner's.
 *
...
object Login {
     ...
     *
     * Creates the home first: both CLIs fail outright on a directory that isn't there, and an
     * account that has never been signed in doesn't have one yet.
     */
    ...
    fun env(account: Account): Map<String, String> = when (account.provider) {
        ...
        Provider.OpenAI -> mapOf("HOME" to account.home)
    }
